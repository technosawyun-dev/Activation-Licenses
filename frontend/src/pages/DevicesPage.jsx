      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {devices.length === 0 ? (
                <tr><td colSpan={9} className="td text-center text-on-surface-variant text-sm py-12">
                ...
                <tr key={d.id} className="hover:bg-[#272a2c] transition-colors border-b cursor-pointer group" style={{ borderColor: 'rgba(255,255,255,0.04)' }} onClick={() => navigate(`/devices/${d.id}`)}>
                  ...
                  <td className="td text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {d.status !== 'blocked' ? (
                        <button onClick={(e) => { e.stopPropagation(); handleBlock(d); }} className="btn-danger" style={{ padding: '4px 8px', fontSize: 12 }}>
                          <span className="msym" style={{ fontSize: 14 }}>block</span>
                        </button>
                      ) : (
                        <button onClick={(e) => { e.stopPropagation(); handleUnblock(d); }} className="btn-ghost" style={{ padding: '4px 8px', fontSize: 12 }}>
